...
import { VALID_DISPLAY_TALENT_DOCUMENT_TYPES } from "helpers/constants";
...
const Resumes = (props) => {
  ...
  useEffect(() => {
    ...
    jobDocuments.map((item,index)=>{
      if (VALID_DISPLAY_TALENT_DOCUMENT_TYPES.includes(item.documentType)) {
        arr.push(item);
      }
    });
        ...
        <Table>
          ...
          <TableBody>
            ...
            {jobDocumentsFillter.map((item, index) => {
              ...
              // let length = jobDocumentsFillter.length;
                  ...
                  <React.Fragment key={index}>
                    <TableRow key={item.documentId} className="text-center bg-gray-100">
                      ...
                      <TableCell align="center">
                        <a href={item.downloadUri} target="_blank" style={{width:30}} title="Download File" download rel="noreferrer">
                          <img src={downloadIcon}
                               style={{height:20,width:20,resize:'contain'}} />
                        </a>
                      </TableCell>
                    </TableRow>
